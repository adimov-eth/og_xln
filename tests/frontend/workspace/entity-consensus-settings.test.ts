...
const replicaFixture = (): EntityReplica => ({
  ...
  certifiedFrameHead: { frame: { height: 8, hash: '0xcertified8' } } as EntityReplica['certifiedFrameHead'],
...
describe('entity consensus settings projection', () => {
  test('projects board, votes, frames, accounts and hooks without reading runtime internals', () => {
    ...
    expect(view.certifiedAnchorHeight).toBe(8);
    expect(view.certifiedAnchorHash).toBe('0xcertified8');
  });
