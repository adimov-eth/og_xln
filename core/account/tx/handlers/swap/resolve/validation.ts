...
import type { SwapResolveFailure, SwapResolveTx, ValidatedSwapResolve } from './types';
...
const resolveCanonicalOffer = (
  ...
  if (offer.crossJurisdiction) {
    return failure(events, 'Cross-jurisdiction offers settle through requestCrossJurisdictionClear/cross_pull_close');
  }
  ...
  const restingMismatch =
    ...
    (data.restingQuantizedGive !== undefined && data.restingQuantizedGive !== canonical.quantizedGive) ||
    (data.restingQuantizedWant !== undefined && data.restingQuantizedWant !== canonical.quantizedWant) ||
  ...
  if (canonical.quantizedGive > canonical.give || canonical.quantizedWant > canonical.want) {
...
const deriveExecutionFill = (
  ...
  const { fillRatio, executionGiveAmount, executionWantAmount, fillNumerator, fillDenominator } = tx.data;
  ...
  if (executionProvided && (executionGiveAmount === undefined || executionWantAmount === undefined)) {
    return failure(events, 'executionGiveAmount and executionWantAmount must both be provided');
  }
  ...
  if (exactRatioProvided && (fillNumerator === undefined || fillDenominator === undefined)) {
    return failure(events, 'fillNumerator and fillDenominator must both be provided');
  }
  if (exactRatioProvided && (fillDenominator! <= 0n || fillNumerator! < 0n || fillNumerator! > fillDenominator!)) {
    return failure(events, `Exact fill ratio out of range: ${fillNumerator}/${fillDenominator}`);
  }
  if (exactRatioProvided && fillNumerator! * offer.quantizedGive !== filledGive * fillDenominator!) {
    return failure(
      events,
      `Exact fill ratio mismatch: ${fillNumerator}/${fillDenominator} ` + `!= ${filledGive}/${offer.quantizedGive}`,
    );
  }
  return {
    filledGive,
...
const validateExecutionEconomics = (
  ...
  if (fill.executionProvided) {
    ...
    if (tx.data.fillRatio !== fill.canonicalFillRatio) {
      return failure(
        events,
        `fillRatio ${tx.data.fillRatio} does not match canonical execution ratio ` + `${fill.canonicalFillRatio}`,
      );
    }
  }
  if (fill.executionProvided && (fill.filledGive > 0n || fill.filledWant > 0n)) {
    if (fill.filledGive > canonical.quantizedGive) {
      return failure(events, `Execution give amount ${fill.filledGive} exceeds offer limit ${canonical.quantizedGive}`);
    }
    if (fill.filledWant * canonical.quantizedGive < fill.filledGive * canonical.quantizedWant) {
...
const validateSwapFeeAuthorization = (
  ...
  if (feeAmount > 0n && feeTokenId !== canonical.offer.wantTokenId) {
    return failure(events, `Swap taker fee token mismatch: expected ${canonical.offer.wantTokenId}, got ${feeTokenId}`);
  }
  if (feeAmount >= fill.filledWant && fill.filledWant > 0n) {
    return failure(events, `Swap taker fee ${feeAmount} exceeds or equals filled receive amount ${fill.filledWant}`);
  }
  try {
    assertSwapNetAuthorization(canonical.offer, fill.filledGive, fill.filledWant, feeAmount, tx.data.cancelRemainder);
  } catch (error) {
...
const validateFilledAmountBounds = (fill: ExecutionFill, events: string[]): SwapResolveFailure | null => {
  if (
    fill.canonicalFillRatio > 0 &&
    (fill.filledGive < FINANCIAL.MIN_PAYMENT_AMOUNT || fill.filledGive > UINT256_MAX)
  ) {
    return failure(
      events,
      `Filled give amount out of bounds: ${fill.filledGive} ` +
        `(min ${FINANCIAL.MIN_PAYMENT_AMOUNT}, max ${UINT256_MAX})`,
    );
  }
  if (
    fill.canonicalFillRatio > 0 &&
    (fill.filledWant < FINANCIAL.MIN_PAYMENT_AMOUNT || fill.filledWant > UINT256_MAX)
  ) {
    return failure(
      events,
