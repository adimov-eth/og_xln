...
test('dev link banner prints stable subsystem links and bulk import fragments', () => {
  ...
  try {
    const keys = spawnSync('bun', [
      'core/scripts/operations/development/dev-radapter-keys.ts',
      '--web-port',
      '8084',
      '--manager-origin',
      'http://localhost:8085',
    ...
    const bannerArgs = [
      'core/scripts/operations/development/print-dev-links.ts',
      '--web-port',
      '8084',
      '--web-http-port',
      '8085',
...
test('bun run dev does not print token-bearing runtime import URLs by default', () => {
  ...
  expect(devChild).toContain('core/orchestrator/orchestrator.ts');
