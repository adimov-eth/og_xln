...
describe('test artifact cleanup', () => {
  ...
  test('nested runner can refresh e2e artifacts only through its inherited run lease', () => {
    ...
    const wrapperPath = join(repoRoot, 'core/scripts/e2e/runners/run-with-test-cleanup.ts');
    const cleanupModulePath = join(repoRoot, 'core/scripts/e2e/harness/test-artifact-cleanup.ts');
    const nestedProbe = [
  ...
  test('SIGKILL of the wrapper cannot expose artifacts while its leased child is alive', async () => {
    ...
    const repoRoot = process.cwd();
    const wrapperPath = join(repoRoot, 'core/scripts/e2e/runners/run-with-test-cleanup.ts');
    const cleanupPath = join(repoRoot, 'core/scripts/e2e/harness/test-artifact-cleanup.ts');
  ...
  test('package test shortcuts run through cleanup before direct browser or hardhat tests', () => {
    ...
    const scenarioRunner = readFileSync(join(repoRoot, 'core/scenarios/run.ts'), 'utf8');
    ...
    expect(rootPackage).toContain('run-with-test-cleanup.ts --reason=persistence-cli -- bun core/scripts/operations/persistence/persistence-wal-smoke.ts');
    expect(rootPackage).toContain('run-with-test-cleanup.ts --reason=watchtower-smoke -- bun core/scripts/operations/production/watchtower-smoke.ts');
    expect(rootPackage).toContain('run-with-test-cleanup.ts --reason=rpc-settlement -- bun core/scripts/operations/settlement/rpc-settlement-parity.ts');
    ...
    for (const scriptName of [
      ...
      expect(rootScripts[scriptName]).toStartWith('bun core/scripts/e2e/runners/run-with-test-cleanup.ts ');
      expect(rootScripts[scriptName]).not.toContain('test-artifact-cleanup.ts');
    }
    expect(rootPackage).toContain('"test:scenarios:parallel:isolated": "bun core/scenarios/run.ts"');
    expect(rootPackage).toContain('"test:feedback:20": "bun core/scenarios/run.ts --set=smoke --workers=2"');
