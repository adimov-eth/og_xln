...
describe('JAdapter watcher ingress', () => {
  ...
  test('J-event observation diagnostics use structured logging only', () => {
    const source = readFileSync(
      join(process.cwd(), 'core/jurisdiction/adapter/events/event-observation.ts'),
      'utf8',
    );
  ...
  test('jadapter ingress depends on the machine queue, not the runtime facade', () => {
    ...
    const source = files
      .map(file => readFileSync(join(process.cwd(), 'core/jurisdiction/adapter', file), 'utf8'))
      .join('\n');

    expect(source).toContain("from '../../../runtime/mempool/input-queue'");
    expect(source).toContain("from '../../../../runtime/mempool/input-queue'");
