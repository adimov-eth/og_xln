...
const readRpcAdapterSource = (): string => [
  ...
  'rpc/rpc-adapter.ts',
  'rpc/rpc-chain-io.ts',
  'rpc/rpc-contract-stack.ts',
  'rpc/rpc-lifecycle.ts',
  'rpc/rpc-receipts.ts',
  'rpc/rpc-reads.ts',
  'rpc/write/rpc-submission.ts',
  'rpc/wallet/rpc-wallet-writes.ts',
  'rpc/watcher/rpc-watcher-controller.ts',
  'rpc/watcher/rpc-watcher-poll.ts',
  'rpc/write/rpc-write-methods.ts',
].map(file => readFileSync(join(process.cwd(), 'core/jurisdiction/adapter', file), 'utf8')).join('\n');
...
test('runtime dev startup status logs stay structured', () => {
  const runtime = readFileSync(join(process.cwd(), 'core/runtime/composition.ts'), 'utf8');
  const runtimeLoop = readFileSync(join(process.cwd(), 'core/runtime/loop/loop.ts'), 'utf8');
  ...
  const retiredLocalConfigPath = join(process.cwd(), 'core/jurisdiction/adapter/local-config.ts');
  const logger = readFileSync(join(process.cwd(), 'core/support/logger.ts'), 'utf8');
