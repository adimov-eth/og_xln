...
} from '../../bootstrap/bootstrap-progress-deadline';
import { createHttpDrainTracker, stopServerGracefully } from '../../graceful-server';
import { reportManagedChildFatal } from '../../process/managed-child-fatal-ipc';
import { deriveMarketMakerChildReadiness } from '../health/market-maker-child-readiness';
import {
  BOOTSTRAP_POLL_MS,
  getAccountReplica,
...
const buildBootstrapStallCapsule = ({
  ...
  return {
    ...
    // waiting for a sibling apart from a lane in retry backoff.
...
const createMarketMakerHttpHandler =
  ...
  async (request, server) => {
    ...
    try {
      ...
      if (pathname === '/api/control/core/quiesce' && request.method === 'POST') {
  ...
  const assertNotStalled = (health: MarketMakerHealth | null): void => {
    ...
    throw new Error(
      `MARKET_MAKER_BOOTSTRAP_STALLED:phase=${deps.phase()}:idleMs=${evaluation.idleMs}:` +
        `pendingNetworkOutputs=${deps.env.pendingNetworkOutputs?.length ?? 0}`,
    );
  };
