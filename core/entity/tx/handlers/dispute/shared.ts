...
import { type OptionalDisputeArgumentWarning } from '../../../../jurisdiction/machine/batch';
import { type DepositoryHankoDomain } from '../../../../protocol/dispute/proof-builder';
...
const requireProofBodyStruct = (
  ...
  if (!isProofBodyStruct(value)) {
    throw haltRuntimeFailure(
      'DISPUTE_FINALIZE_PROOFBODY_INVALID',
...
const toBigIntStrict = (value: unknown, label: string): bigint => {
  ...
  throw haltRuntimeFailure(
    'DISPUTE_FINALIZE_PROOFBODY_VALUE_INVALID',
    `DISPUTE_FINALIZE_PROOFBODY_VALUE_INVALID:${label}`,
  );
};
...
const requireBytesLike = (value: unknown, label: string): string => {
  if (typeof value !== 'string' || !value.startsWith('0x')) {
    throw haltRuntimeFailure(
      'DISPUTE_FINALIZE_PROOFBODY_BYTES_INVALID',
      `DISPUTE_FINALIZE_PROOFBODY_BYTES_INVALID:${label}`,
    );
  }
  return value;
};
...
const requireAddressLike = (value: unknown, label: string): string => {
  if (!isUsableContractAddress(value)) {
    throw haltRuntimeFailure(
      'DISPUTE_FINALIZE_PROOFBODY_ADDRESS_INVALID',
      `DISPUTE_FINALIZE_PROOFBODY_ADDRESS_INVALID:${label}`,
    );
  }
  return value;
};
...
const requireResponseSeconds = (value: unknown, label: string): bigint => {
  ...
  if (seconds < 0n || seconds > 0xffff_ffffn) {
    throw haltRuntimeFailure(
      'DISPUTE_FINALIZE_PROOFBODY_RESPONSE_SECONDS_INVALID',
      `DISPUTE_FINALIZE_PROOFBODY_RESPONSE_SECONDS_INVALID:${label}`,
    );
  }
  return seconds;
};
...
export const canonicalizeProofBodyStruct = (
  ...
  const leftResponseSeconds = requireResponseSeconds(proofBody.leftResponseSeconds, `${source}.leftResponseSeconds`);
  const rightResponseSeconds = requireResponseSeconds(proofBody.rightResponseSeconds, `${source}.rightResponseSeconds`);
  if (leftResponseSeconds + rightResponseSeconds > 365n * 24n * 60n * 60n) {
    throw haltRuntimeFailure(
      'DISPUTE_FINALIZE_PROOFBODY_RESPONSE_TOTAL_INVALID',
      `DISPUTE_FINALIZE_PROOFBODY_RESPONSE_TOTAL_INVALID:${source}`,
    );
  }
  return {
    ...
    offdeltas: proofBody.offdeltas.map(entry => encodeInt512(decodeInt512(entry))),
...
export const resolveDepositoryHankoDomain = (entityState: EntityState): DepositoryHankoDomain | null => {
  ...
  if (!Number.isSafeInteger(chainId) || chainId <= 0) {
    throw haltRuntimeFailure(
      'DISPUTE_HANKO_CHAIN_ID_INVALID',
      `DISPUTE_HANKO_CHAIN_ID_INVALID:${String(jurisdiction?.chainId)}`,
    );
  }
...
const hasQueuedDisputeOperation = (
  ...
  return (
    draft.some(matches) ||
    sent.some(matches) ||
    (state.jBatchState?.recoveryBatches ?? []).some(batch => batch[kind].some(matches))
  );
};

export const hasQueuedDisputeStart = (state: EntityState, counterpartyEntityId: string): boolean =>
  hasQueuedDisputeOperation(state, counterpartyEntityId, 'disputeStarts');

export const hasQueuedDisputeFinalize = (state: EntityState, counterpartyEntityId: string): boolean =>
  hasQueuedDisputeOperation(state, counterpartyEntityId, 'disputeFinalizations');

export const collectDisputeEvidenceReadinessIssues = (account: AccountReplica, now: number): string[] => {
  const issues: string[] = [];
