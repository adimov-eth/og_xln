...
const generateHtlcEphemeralPrivateKey = (deterministicTestSeed?: string): string => {
  const digest = deterministicTestSeed === undefined
    ? undefined
    : keccakBytesHash(new TextEncoder().encode(deterministicTestSeed)).slice(2);
...
const assertOriginRouteEvidence = (
  ...
  if (tx.data.route.length > 0 && !canonicalConsensusValuesEqual(route, tx.data.route)) {
...
export const assertOriginatedHtlcPayments = (input: AssertOriginatedInput): void => {
  ...
  for (const { tx, jHeight } of expectedTxs) {
    ...
    if (input.state.paybook.entries.has(origin.hashlock) || proposalHashlocks.has(origin.hashlock)) {
      rejectHtlcPayment(`HTLC_PAYMENT_HASHLOCK_ALREADY_ACTIVE:${origin.hashlock}`);
    }
