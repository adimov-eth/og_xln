...
import { validateStorageFrameRecordValue, validateStorageHeadValue } from '../schema/authoritative-schema';
...
type ConcreteCheckpointExportDeps = Pick<RuntimeStorageApiDeps, 'getStorageDb' | 'getRuntimeWalDb'>;

const STATE_TAGS = new Set([
  0x17, 0x18, 0x19, 0x21, 0x22, 0x23, 0x24, 0x26, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f, 0x30, 0x36, 0x37, 0x38,
]);
...
const addCanonicalEmptyAccountAuthority = (rows: Map<string, Buffer>, ownerHex: string): void => {
  const accountTags = new Set([0x17, 0x18, 0x19]);
  const present = [...rows.keys()].filter(
    key => key.slice(2, 66) === ownerHex && accountTags.has(Number.parseInt(key.slice(0, 2), 16)),
  );
  ...
  const signerDigest = createHash('sha256').update('xln.rscore.signer-config.v1').digest('hex');
  rows.set(
    keyRscoreCheckpoint(ownerEntityId).toString('hex'),
    encodeBuffer({
      version: 1,
      ownerEntityId,
...
const readStateRows = async (
  currentDb: RuntimeDbLike,
  walDb: RuntimeDbLike,
  expectedOwners: ReadonlySet<string>,
): Promise<HexRow[]> => {
  ...
  const add = async (db: RuntimeDbLike, key: Buffer): Promise<void> => {
    ...
    const owner = key.subarray(1, 33).toString('hex');
    if (!expectedOwners.has(owner)) throw new Error(`CHECKPOINT_STATE_OWNER_UNDECLARED:0x${owner}`);
    owners.add(owner);
    const value = DEDICATED_FIELD_TAGS.has(tag) ? await db.get(key) : await requiredBounded(db, key);
...
const readMachineLeaves = async (walDb: RuntimeDbLike, leafCount: number): Promise<HexRow[]> => {
...
export const exportConcreteCheckpointSource = async (
  ...
  withRuntimeCommittedRead(env, async () => {
    ...
    if (currentMaterializedHeight !== height) {
      throw new Error(`CHECKPOINT_MACHINE_NOT_CURRENT:requested=${height}:current=${currentMaterializedHeight}`);
    }
    ...
    const runtimeMachineLeaves = await readMachineLeaves(walDb, frame.runtimeMachineRoot.leafCount);
    if (!frame.canonicalEntityHashes) throw new Error(`CHECKPOINT_CANONICAL_ENTITIES_MISSING:${height}`);
    const expectedOwners = new Set(frame.canonicalEntityHashes.map(entry => entry.entityId.slice(2)));
    const stateRows = await readStateRows(currentDb, walDb, expectedOwners);
    return {
      height,
