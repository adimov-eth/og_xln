...
const machineGraphLog = createStructuredLogger('storage.machine-graph');
import {
...
/**
 * The decoders already validate shape and return fresh frozen segments/atoms;
 * a msgpack round trip on every leaf key and value only repeated that copy at
 * ~10x the cost, once per atom of every materialized Account and Runtime doc.
 */
...
export const prepareRuntimeMachineGraphRows = (
  ...
  if (isRuntimePerfProfileEnabled('XLN_STORAGE_MACHINE_PROFILE')) {
    machineGraphLog.info('machine_graph.profile', {
      ...
      topLevel: Object.entries(machine).map(([key, value]) => ({
        key,
        bytes: encodeBuffer(value, { omitSymbolKeys: true }).byteLength,
