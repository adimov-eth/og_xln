...
export class RuntimeWsClient {
  ...
  constructor(options: RuntimeWsClientOptions) {
    ...
    failfastAssert(!!options.helloAudience, 'WS_INIT_AUDIENCE_MISSING', 'RuntimeWsClient audience is required');
    failfastAssert(
  ...
  private sendHello(challengeMessage?: RuntimeWsMessage): boolean {
    ...
    try {
      ...
      const ephemeral = expectedAudience.startsWith('xln-runtime:') ? generateEphemeralKeyPair() : null;
      const sessionPubKey = ephemeral ? pubKeyToHex(ephemeral.publicKey) : undefined;
      const signature = signDigest(
        this.options.seed,
        this.options.signerId,
        hashHelloMessage(this.options.runtimeId, encryptionPubKey, timestamp, nonce, expectedAudience, sessionPubKey),
      );
      ...
      this.sessionEphemeral = ephemeral;
      this.helloSent = true;
      return true;
    } catch (error) {
  ...
  private handleHandshakeMessage(msg: RuntimeWsMessage): boolean {
    ...
    if (msg.type === 'hello_ack') {
      ...
      const directPeerRuntimeId = this.helloAudience?.startsWith('xln-runtime:')
        ...
        : '';
      if (directPeerRuntimeId) {
        const peerKey = typeof msg.fromEncryptionPubKey === 'string' ? msg.fromEncryptionPubKey.toLowerCase() : '';
        const authError = normalizeRuntimeId(msg.from || '') !== directPeerRuntimeId || !peerKey
          ? 'Direct hello acknowledgement identity missing or mismatched'
        ...
        if (authError) {
          this.options.onError?.(new Error(`WS_DIRECT_SERVER_AUTH_INVALID:${authError}`));
          this.ws?.close();
          return true;
        }
        this.directPeerEncryptionPubKey = peerKey;
        this.lastInboundAuthTimestamp = msg.auth!.timestamp;
        const serverSessionPubKey = typeof msg.sessionPubKey === 'string' ? msg.sessionPubKey.toLowerCase() : '';
        if (this.sessionEphemeral && !this.helloAcknowledged) {
          // The server's ephemeral key arrives inside its runtime-signed
          // hello_ack (verified above); a server that ignores the offer stays
          // on per-frame signatures.
          if (serverSessionPubKey) {
            this.sessionKeys = deriveRuntimeWsSessionKeys(
              x25519SharedSecret(this.sessionEphemeral.privateKey, hexToPubKey(serverSessionPubKey)),
              this.helloNonce!,
              this.helloAudience!,
            );
          }
  ...
  private verifyInboundDirectFrame(msg: RuntimeWsMessage): boolean {
    ...
    const verifiedError = normalizeRuntimeId(msg.from || '') !== directPeerRuntimeId
      || peerKey !== this.directPeerEncryptionPubKey
      ? 'Direct server identity or encryption key mismatch'
      : verifyRuntimeWsFrameAuth(
          directPeerRuntimeId,
          msg,
          msg.auth,
          ...
          this.sessionKeys?.s2c,
        );
    if (!verifiedError) {
      this.lastInboundAuthTimestamp = msg.auth!.timestamp;
      return true;
    }
    this.options.onError?.(new Error(`WS_DIRECT_SERVER_AUTH_INVALID:${verifiedError}`));
    this.ws?.close();
    return false;
  }
  ...
  private async handleMessage(
    ...
    if (!this.verifyInboundDirectFrame(msg)) return;
    if (typeof msg.from === 'string' && typeof msg.fromEncryptionPubKey === 'string') {
      this.options.onPeerEncryptionKey?.(msg.from, msg.fromEncryptionPubKey);
    }
  ...
  private sendRaw(msg: RuntimeWsMessage, accountEnvelope?: RuntimeEntityInputsEnvelope): boolean {
    ...
    try {
      ...
      if (outboundMsg.type !== 'hello') {
        ...
        outboundMsg.auth = this.sessionKeys
          ? {
              nonce: this.helloNonce!,
              timestamp: authTimestamp,
              mac: macRuntimeWsFrame(this.sessionKeys.c2s, outboundMsg, this.helloAudience!, this.helloNonce!, authTimestamp),
            }
          : {
              nonce: this.helloNonce!,
              timestamp: authTimestamp,
