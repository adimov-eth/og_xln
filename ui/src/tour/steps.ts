...
export const TOUR_STEPS: TourStep[] = [
	...
	},
	{
		...
		body: 'The other side can dispute too, with an old page that favours them, hoping you are asleep. You get a window to answer with the newer page. A tower is a service you pick that keeps an encrypted copy of your latest pages, so any wallet can pick your accounts up from your phrase alone. It cannot read your balances, and you can run your own. Open the shield: Watchtower says whether anyone is holding a copy for you, and lets you name one.',
		more: 'Blind encrypted backup: the tower sees a lookup key derived from your seed, the ciphertext and its size, nothing else. Appointing a tower to counter-dispute for you while you are away is a second, separate service this wallet cannot sign yet, and the Watchtower card says so instead of implying cover it does not have.',
		target: ctx => (at(ctx, '/sovereignty') ? 'sovereignty-watchtower' : viaHome(ctx, 'home-sovereignty')),
		hint: ctx => (at(ctx, '/sovereignty') ? 'Read who is watching, and what a backup does and does not cover.' : homeHint(ctx, 'Tap the shield.')),
		mode: 'read',
	},
	{
		id: 'evidence',
