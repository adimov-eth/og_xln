...
| 9 | Runtime atomic admission of paired legs, sibling cohort gating (`selectCrossJOpeningAccountProposalTxs`) | runtime/frame/cross-j/atomic-admission.ts | not ported | REMAINING. This is runtime frame admission, owned by the runtime owner. Each Entity's committed state already MATCHes once its frame is applied. |
