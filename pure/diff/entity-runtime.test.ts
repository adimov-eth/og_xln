...
import {
  ...
  hashEntityFrame, hashEntityState, isSingleSigner, leaderOrder, replicaKey, spawn, signature, tokenId, ZERO_WORD,
...
describe("entity-runtime: entity tx fold (ER-7, ER-12, ER-13, ER-14)", () => {
  ...
  test("MATCH (og createInboundAccountState): the peer opens its side from the first Account frame; no openAccount output is needed", () => {
    ...
    const outbox = run([{ kind: "create", entityId: ALICE, signerId: aliceAddr, input: txs([open], NOW) }]);
