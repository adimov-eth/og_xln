...
import EntityPanelShell from "../EntityPanelShell.svelte";
...
function moveNodeAction(node: HTMLButtonElement, params: { side: "from" | "to"; endpoint: MoveEndpoint }): { update: (next: { side: "from" | "to"; endpoint: MoveEndpoint }) => void; destroy: () => void } {
...
function moveRouteSteps(from: MoveEndpoint, to: MoveEndpoint): string[] {
  ...
  const reserveRecipientLabel = moveReserveRecipientEntityId ? formatAddress(moveReserveRecipientEntityId) : "recipient reserve";
  return buildMoveRouteSteps(from, to, {
    targetEntityLabel,
    targetHubLabel,
    reserveRecipientLabel,
    hasRemoteReserveRecipient: Boolean(moveReserveRecipientEntityId && moveReserveRecipientEntityId !== resolveSelfEntityId()),
  });
}
...
$: moveAllowanceRouteEnabled = assetWorkspaceTab === "move" && activeCommandsReady && routeRequiresExplicitExternalAllowance(moveFromEndpoint, moveToEndpoint);
...
$: moveAllowanceAssetIdentity = String(moveAssetSymbol || "")
  .trim()
  .toUpperCase();
$: moveAllowanceToken = moveAllowanceAssetIdentity ? findPanelExternalToken(moveAllowanceAssetIdentity) : null;
$: moveAllowanceMetadataLoading = moveAllowanceRouteEnabled && Boolean(moveAllowanceAssetIdentity) && externalTokensLoading && !moveAllowanceToken;
...
$: moveAllowanceStatusLabel = buildMoveAllowanceStatusLabel({
  ...
  tokenDecimals: moveAllowanceToken ? requireTokenDecimals(moveAllowanceToken.decimals, `asset:${moveAllowanceAssetIdentity}`) : null,
...
function resolveSelfEntityId(): string {
  return String(replica?.state?.entityId || tab.entityId || "")
    .trim()
    .toLowerCase();
}
function handleMoveReserveRecipientChange(event: CustomEvent<{ value?: string }>) {
  moveReserveRecipientEntityId = String(event.detail?.value || "")
    .trim()
    .toLowerCase();
}
...
function getCurrentMoveSourceAccountId(): string {
  ...
  return selectPreferredMoveSourceAccountId(token.tokenId, getRequestedMoveAmount(token));
}
function getCurrentMoveTargetEntityId(): string {
  return String(moveTargetEntityId || resolveSelfEntityId() || "")
    .trim()
    .toLowerCase();
}
function getCurrentMoveTargetHubId(): string {
  return String(moveTargetHubEntityId || workspaceAccountId || selectedAccountId || "")
    .trim()
    .toLowerCase();
}
...
function selectPreferredMoveSourceAccountId(tokenId: number, requestedAmount: bigint): string {
  return getPreferredMoveSourceAccountId({
...
function computeMoveSourceAvailableBalance(row: AssetLedgerRow | null, liveTransferToken: ReserveTransferAsset | null): bigint {
  return computeMoveSourceAvailableBalanceForEndpoint({
    from: moveFromEndpoint,
    row,
    liveTransferToken,
    externalToken: findPanelExternalToken(moveAssetSymbol),
    reserveBalance: (tokenId) => onchainReserves.get(tokenId) ?? 0n,
    draftReserveDelta: getMoveDraftReserveDelta,
    outgoingDebt: getOpenOutgoingDebtForToken,
    sourceAccountId: getCurrentMoveSourceAccountId(),
...
function selectPreferredMoveAssetSymbol(): string {
  const sourceAccountId = getCurrentMoveSourceAccountId();
  return choosePreferredMoveAssetSymbol({
    candidates: moveAssetOptions,
    availableBalance: (symbol) => {
      const externalToken = findPanelExternalToken(symbol);
      const reserveToken = findReserveTransferTokenBySymbol(symbol);
      return getMoveMaxAmount(moveFromEndpoint, reserveToken, externalToken, sourceAccountId) ?? 0n;
    },
  });
}
...
function resolveHubDiscoveryAvatar(entityId: string): string {
  return entityAvatar(activeXlnFunctions, entityId);
}
...
$: avatar = entityAvatar(activeXlnFunctions, tab.entityId);
$: activeEnv = env;
...
$: currentEntityJurisdictionName = getCurrentEntityJurisdictionName(null, replica) ?? panelView.activeJurisdictionName ?? tab.jurisdiction ?? null;
...
$: gossipName = (() => {
  ...
  if (!entityId) return "";
  const profile = panelProfiles.find((p: Profile) => p.entityId.toLowerCase() === entityId);
  return profile?.name || "";
})();
$: heroDisplayName = (() => {
...
function resolveEntitySigner(entityId: string, reason: string): string {
  ...
  const normalizedEntityId = String(entityId || "")
    .trim()
    .toLowerCase();
  const projectionEntityId = String(replica?.state?.entityId || replica?.entityId || tab.entityId || "")
    .trim()
    .toLowerCase();
...
async function saveSettingsProjectionProfile(draft: EntitySettingsProfileDraft): Promise<void> {
  const entityId = String(currentEntityValue || replica?.state?.entityId || tab.entityId || "")
    .trim()
    .toLowerCase();
...
$: if (assetWorkspaceTab === "move" && workspaceAccountIds.length > 0) {
  ...
  if (moveFromEndpoint === "account" && token) {
    const preferred = selectPreferredMoveSourceAccountId(token.tokenId, getRequestedMoveAmount(token));
...
$: moveLayoutSignature = [assetWorkspaceTab, moveFromEndpoint, moveToEndpoint, moveSelectedSource || "", moveSelectedTarget || "", moveNeedsReserveRecipient(moveFromEndpoint, moveToEndpoint) ? "reserve-recipient" : "", moveNeedsExternalRecipient(moveFromEndpoint, moveToEndpoint) ? "external-recipient" : "", moveToEndpoint === "account" ? "target-account" : ""].join("|");
...
function findPanelExternalToken(symbol: string): ExternalToken | null {
  return nullOnAmbiguousAsset(() => findExternalTokenBySymbol(externalTokens, symbol));
}
function findPanelAssetLedgerRow(symbol: string): AssetLedgerRow | null {
  return nullOnAmbiguousAsset(() => findAssetLedgerRowBySymbol(assetLedgerRows, symbol));
}
function findReserveTransferTokenBySymbol(symbol: string): ReserveTransferAsset | null {
  return nullOnAmbiguousAsset(() =>
    resolveReserveTransferTokenBySymbol({
      symbol,
      externalTokens,
      assetLedgerRows,
...
function getPanelFaucetReserveTokenMeta(symbol: string): { tokenId: number; symbol: string } | null {
  return nullOnAmbiguousAsset(() => getFaucetReserveTokenMeta(assetLedgerRows, symbol));
}
function requirePanelExternalToken(symbol: string): ExternalToken {
  return requireExternalTokenBySymbol(externalTokens, symbol);
}
...
function getDerivedDeltaForAccount(counterpartyEntityId: string, tokenId: number) {
  ...
  const entityId = String(replica?.state?.entityId || tab.entityId || "")
    .trim()
    .toLowerCase();
  const counterpartyId = String(counterpartyEntityId || "")
    .trim()
    .toLowerCase();
...
async function resolveCurrentExternalAddress(): Promise<string> {
  ...
  return new Wallet(hexlify(privKey)).address;
}
    ...
    await submitEntityInputs([
      {
        entityId,
        signerId,
        entityTxs: [buildReserveToExternalEoaTx(externalAddress, tokenId, amount), buildBroadcastTx()],
      },
    ]);
...
async function reserveToReserve(tokenId: number, amount: bigint, recipientEntityIdOverride?: string): Promise<void> {
  ...
  const recipientEntityId = String(recipientEntityIdOverride || moveReserveRecipientEntityId || "")
    .trim()
    .toLowerCase();
  ...
  await submitEntityInputs([
    {
      entityId,
      signerId,
      entityTxs: [buildReserveToReserveTx(recipientEntityId, tokenId, amount), buildBroadcastTx()],
    },
  ]);
}
async function faucetReserves(tokenId: number = 1, symbolHint?: string) {
  ...
  try {
    ...
    const existingForToken = pendingReserveFaucets.filter((req) => req.tokenId === tokenMeta.tokenId).sort((a, b) => b.startedAt - a.startedAt)[0];
...
async function handleQuickSettleApprove(event: CustomEvent<{ counterpartyId: string; workspaceHash: string }>) {
  ...
  try {
    ...
    await submitEntityInputs([buildEntityInput(entityId, signerId, [buildSettlementApproveTx(event.detail.counterpartyId, event.detail.workspaceHash)])]);
...
const getTokenList = createExternalTokenCatalogLoader(() => fetchExternalTokenCatalog(resolveApiBase()));
$: {
  activeEnv;
  envRevision;
  replica;
...
$: {
  if (pendingAssetBridgeSync && !resolvingAssetBridgeSync) {
    ...
    const reserveMoved = pendingAssetBridgeSync.direction === "deposit" ? currentReserve > pendingAssetBridgeSync.baselineReserve : currentReserve < pendingAssetBridgeSync.baselineReserve;
    if (reserveMoved) {
...
$: {
  ...
  const symbol = String(moveAssetSymbol || "")
    .trim()
    .toUpperCase();
  const row = symbol ? findPanelAssetLedgerRow(moveAssetSymbol) : null;
  const selectedDecimals = row?.decimals ?? selectedMoveExternalToken?.decimals ?? selectedMoveTransferToken?.decimals;
  const nextState: MoveUiState = {
    ...
    displayDecimals: selectedDecimals === undefined ? 0 : requireTokenDecimals(selectedDecimals, `asset:${moveAssetSymbol}`),
...
$: {
  ...
  const preferredFaucetSymbol = assetLedgerRows.find((row) => !row.isNative && typeof row.tokenId === "number" && row.tokenId > 0)?.symbol ?? preferred;
  if (!findPanelAssetLedgerRow(faucetAssetSymbol)) faucetAssetSymbol = preferredFaucetSymbol;
  if (!findPanelExternalToken(sendAssetSymbol)) sendAssetSymbol = preferred;
  ...
  const movePreferred = selectPreferredMoveAssetSymbol();
  if (
    !moveAssetOptions.some(
      (token) =>
        token.symbol.toUpperCase() ===
        String(moveAssetSymbol || "")
          .trim()
          .toUpperCase(),
    )
  ) {
...
$: {
  const moveAmountContextKey = [assetWorkspaceTab, accountWorkspaceTab, moveFromEndpoint, moveToEndpoint, moveAssetSymbol, getCurrentMoveSourceAccountId()].join("|");
...
$: if (pendingReserveFaucets.length > 0) {
  const now = Date.now();
  const { remaining, received, timedOut } = reconcilePendingReserveFaucets(pendingReserveFaucets, now, (tokenId) => onchainReserves.get(tokenId) ?? 0n);
...
async function fetchExternalTokens(forceSnapshot = false) {
  ...
  externalFetchInFlight = (async () => {
    ...
    try {
      ...
      const entityId = resolveSelfEntityId();
      const spender = resolveExternalWalletSpender(jadapter, jurisdiction, panelView.jurisdictions ?? []);
      const allowanceReads = moveAllowanceRouteEnabled && isAddress(spender) && spender !== ZeroAddress ? tokenList.filter((token) => isAddress(token.address) && token.address !== ZeroAddress).map((token) => ({ tokenAddress: token.address, spender })) : [];
      ...
      if (observed) {
        ...
        if (observed.sourceHeight !== undefined) {
          snapshotSource = {
            ...
            ...(externalWalletSnapshotSource?.sourceHeight === observed.sourceHeight && externalWalletSnapshotSource.sourceHash ? { sourceHash: externalWalletSnapshotSource.sourceHash } : {}),
            ...(externalWalletSnapshotSource?.sourceHeight === observed.sourceHeight && externalWalletSnapshotSource.finalityDepth !== undefined ? { finalityDepth: externalWalletSnapshotSource.finalityDepth } : {}),
            ...(externalWalletSnapshotSource?.sourceHeight === observed.sourceHeight && externalWalletSnapshotSource.headBlockNumber !== undefined ? { headBlockNumber: externalWalletSnapshotSource.headBlockNumber } : {}),
          };
        }
      } else if (entityId) {
        const snapshot = await requestExternalWalletSnapshot(resolveApiBase(), entityId, owner, tokenList, allowanceReads, jadapter);
        if (!snapshot) {
          externalTokensLoading = false;
      ...
      tokenList.forEach((token) => {
        const error = tokenErrorByAddress.get(
          String(token.address || "")
            .trim()
            .toLowerCase(),
        );
      ...
      if (message.includes("ENTITY_JURISDICTION_MISSING") || message.includes("ENTITY_JURISDICTION_UNAVAILABLE") || message.includes("J-adapter not available")) {
        externalTokens = [];
        externalWalletSnapshotSource = null;
        if (moveAllowanceRouteEnabled) {
          moveAllowanceRaw = null;
...
function openAssetMoveWorkspace(): void {
  ...
  moveAssetSymbol = selectPreferredMoveAssetSymbol();
...
async function waitForMoveCondition(predicate: () => boolean, label: string, timeoutMs = 20_000, pollMs = 100): Promise<void> {
...
async function queueReserveToExternalDraft(tokenId: number, amount: bigint, recipientEoaOverride?: string): Promise<void> {
  const entityId = String(replica?.state?.entityId || tab.entityId || "")
    .trim()
    .toLowerCase();
  ...
  if (getRuntimeEnv(actionRuntimeEnv)) {
    await waitForMoveCondition(() => {
      ...
      return Array.isArray(batch?.reserveToExternalToken) && batch.reserveToExternalToken.some((op) => Number(op?.tokenId) === tokenId && BigInt(op?.amount || 0n) === amount && String(op?.receivingEntity || "").toLowerCase() === receivingEntity);
    }, "Waiting for reserve withdrawal to appear in draft batch");
  }
}
async function queueReserveToCollateralDraft(tokenId: number, amount: bigint, counterpartyEntityIdOverride?: string, receivingEntityIdOverride?: string): Promise<void> {
  const entityId = String(replica?.state?.entityId || tab.entityId || "")
    .trim()
    .toLowerCase();
  ...
  const receivingEntityId = String(receivingEntityIdOverride || entityId || "")
    .trim()
    .toLowerCase();
...
async function queueExternalToReserveDraft(tokenAddress: string, amount: bigint, internalTokenId?: number): Promise<void> {
  const entityId = String(replica?.state?.entityId || tab.entityId || "")
    .trim()
    .toLowerCase();
...
async function addMoveToExistingBatch(skipValidation = false): Promise<void> {
  ...
  const externalToken = findPanelExternalToken(moveAssetSymbol);
...
async function executeMovePlan(): Promise<void> {
  ...
  const moveTargetAccount = getCurrentMoveTargetHubId();
  const moveTargetEntity = getCurrentMoveTargetEntityId();
  const moveReserveRecipient = String(moveReserveRecipientEntityId || "")
    .trim()
    .toLowerCase();
  moveExecuting = true;
  moveProgressLabel = "";
  try {
    ...
    switch (routeKey) {
      ...
      case "account->reserve":
        ...
        await collateralToReserve(token.tokenId, amount, moveSourceAccount, reserveRecipientIsSelf ? { type: "none" } : { type: "r2r", recipientEntityId: moveReserveRecipient });
        break;
      ...
      case "account->account":
        setMoveProgress("Requesting hub proof and settling account back to your reserve");
        await collateralToReserve(token.tokenId, amount, moveSourceAccount, {
          type: "reserve_to_collateral",
          targetEntityId: moveTargetEntity,
          counterpartyEntityId: moveTargetAccount,
        });
...
async function reserveToCollateral(tokenId: number, amountOverride?: bigint, counterpartyEntityIdOverride?: string, receivingEntityIdOverride?: string) {
  const entityId = replica?.state?.entityId || tab.entityId;
...
async function queueDisputePrepare(counterpartyEntityId: string, description = "dispute-prepare-from-configure") {
  ...
  if (!activeIsLive) {
    toasts.error("Dispute prepare requires LIVE mode");
    return;
  }
  try {
...
async function queueDisputeFinalize(counterpartyEntityId: string, description = "dispute-finalize-from-configure") {
  ...
  try {
    await submitEntityInputs([buildEntityInput(entityId, signerId, [buildDisputeFinalizeTx(counterpartyEntityId, description)])]);
...
async function enforceOutstandingDebt(request: DebtEnforceRequest): Promise<void> {
  ...
  try {
    ...
    toasts.success(`Drain submitted: ${amountLabel} payable now, ${openCount} open debts, ${totalLabel} outstanding, ${reserveLabel} reserve, next ${nextDebtIndex === null ? "—" : `#${nextDebtIndex}`}.`);
  } catch (err) {
...
async function addTokenToAccount() {
  ...
  try {
    await submitEntityInputs([buildEntityInput(entityId, signerId, [buildAddTokenToAccountTx(counterpartyEntityId, configureTokenId)])]);
...
async function submitAssetFaucet(target: "external" | "reserve" | "account"): Promise<void> {
  ...
  try {
    ...
    const token = getPanelFaucetReserveTokenMeta(faucetAssetSymbol);
    if (!token) {
...
onMount(() => {
  ...
  const handleMovePointer = (event: PointerEvent | MouseEvent) => {
    ...
    moveDragHoverTarget = endpoint === "external" || endpoint === "reserve" || endpoint === "account" ? endpoint : null;
  };
...
$: pendingBatchCanBroadcast = canBroadcastPendingBatch(pendingBatchState, pendingBatchReserveIssue);
...
const runPendingBatchAction = createPendingBatchActionRunner({
  getState: () => ({
    ...
    canBroadcastPendingBatch: pendingBatchCanBroadcast,
    hasSentBatch,
  }),
...
$: routeSyncSignature = [activeTab, assetWorkspaceTab, settingsSubview, accountWorkspaceTab].join(":");
$: {
...
<EntityPanelShell panelId={tab.id}>
  <EntityPanelChrome
    {tab} {hideHeader} {showJurisdiction}
    {userModeHeader}
  ...
  <main class="main-scroll">
    ...
    {#if !tab.entityId || !tab.signerId}
      <EntitySelectionEmptyState
        {tab} {userModeHeader} {resettingEverything}
        {allowHeaderAddRuntime} {headerRuntimeAddLabel} {handleResetEverything}
        {handleHeaderAddRuntime} {handleHeaderAddJurisdiction} {handleHeaderAddEntity}
        {handleEntitySelect}
      />
    {:else if isAccountFocused && selectedAccount && selectedAccountId}
      <EntityFocusedAccountView
        {selectedAccount} {selectedAccountId} {tab}
        {replica}
        entityNames={panelView.entityNames}
        {pendingOffchainFaucetKeys}
        commandsReady={activeCommandsReady}
        {handleBackToAccounts} {handleAccountFaucet} {handleAccountPanelGoToOpenAccounts}
      />
    {:else if replica}
      <EntityPanelHeroTabs
      ...
      <section class="content">
        {#if activeTab === 'assets'}
          <EntityAssetsTab
            {replica} {tab}
            activeIsLive={activeCommandsReady} profileByEntityId={panelView.profileByEntityId} entityNames={panelView.entityNames}
            {currentExternalEoaValue} {copiedMetaField} {externalWalletSnapshotSource}
            {externalTokensLoading} {assetFaucetSubmitting} {assetLedgerRows}
            {assetLedgerTotals} {assetLedgerGrandTotal}
            bind:faucetAssetSymbol
            {faucetSupportsReserve} {canShowAccountFaucet} {openOutgoingDebtSummary}
            {pendingBatchCount} {pendingBatchMode} {pendingBatchReserveIssueText}
            {pendingBatchPreview} {pendingBatchSubmitting} {hasSentBatch}
            canBroadcastPendingBatch={pendingBatchCanBroadcast} {assetWorkspaceTab}
            bind:moveAmount bind:moveAssetSymbol bind:moveFromEndpoint
            bind:moveToEndpoint bind:moveExternalRecipient bind:moveReserveRecipientEntityId
            bind:moveSourceAccountId bind:moveTargetEntityId bind:moveTargetHubEntityId
            {moveExecuting} {moveProgressLabel} {moveDraftError}
            {moveBroadcastError} {moveAllowanceRouteEnabled} {moveAllowanceSatisfied}
            {moveAllowanceLoading} {moveAllowanceStatusLabel} {moveAllowanceAmount}
            {moveAllowanceSubmittingMode} {moveSelectedSource} {moveSelectedTarget}
            {moveDragSource} {moveDragHoverTarget} {moveLineReady}
            {moveCommittedLineReady} {moveNodeLayoutVersion} {moveNeedsReserveRecipient}
            ...
            {openAssetMoveWorkspace} {openAssetHistoryWorkspace} {clearPendingBatch}
            {rebroadcastPendingBatch} {broadcastPendingBatch}
          />
        {:else if activeTab === 'accounts'}
          <AccountWorkspaceView
            {replica} {tab} {activeEnv}
            {liveRuntimeEnv}
            activeIsLive={activeCommandsReady}
            {actionRuntimeEnv} {canOpenAccounts} submitRuntimeInput={submitPanelRuntimeInput}
            runtimeHeight={panelView.height} entityNames={panelView.entityNames} profileByEntityId={panelView.profileByEntityId}
            isDevnet={panelView.isDevnet}
            {hubDiscoveryProjection} {paymentView} {swapRuntimeView}
            {accountIds} {workspaceAccountIds} {workspaceAccountId}
            ...
            {moveEntityOptions} {moveHubEntityOptions} {moveSourceAccountOptions}
            {moveAssetOptions}
            moveEndpointLabels={MOVE_ENDPOINT_LABEL} moveEndpoints={MOVE_ENDPOINTS} bind:openAccountEntityId
            {openAccountEntityOptions} {disputedAccounts} {entityActivityRows}
            {filteredEntityActivityRows} {entityActivityAccounts}
        ...
        {:else if activeTab === 'settings'}
          <EntitySettingsProjectionPanel
            entityId={currentEntityValue || tab.entityId} signerId={currentSignerId || tab.signerId} runtimeId={panelView.runtimeId}
            runtimeHeight={panelView.height} jurisdictionLabel={selectedJurisdictionName || ''} profile={replica.state?.profile ?? null}
            hubPolicy={replica.state?.hubRebalanceConfig ?? null} accountCount={accountIds.length} reserveCount={replica.state?.reserves?.size ?? 0}
            proposalCount={replica.state?.proposals?.size ?? 0} isHub={replica.state?.profile?.isHub === true || Boolean((replica.state as { orderbookHubProfile?: unknown })?.orderbookHubProfile)} activeIsLive={activeCommandsReady}
            ...
            {settingsSubview}
            onSaveProfile={saveSettingsProjectionProfile} onImportJMachine={importSettingsJMachine}
          />
        {/if}
      </section>
    {/if}
