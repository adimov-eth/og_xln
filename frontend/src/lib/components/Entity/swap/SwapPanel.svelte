...
const {
  ...
  getRuntime: () => activeXlnFunctions,
  getCurrentReplica: () => currentReplica,
  tokenSymbol,
  getTokenDecimals,
});
...
$: if (!sourceEntityOptions.some((option) => option.value === selectedSourceEntityValue)) {
  const tabEntityId = String(tab.entityId || '')
    .trim()
    .toLowerCase();
...
function isHubAccount(accountIdValue: string): boolean {
  const normalized = String(accountIdValue || '')
    .trim()
    .toLowerCase();
  ...
  swapRouteMode === 'cross'
    ...
    : orderbookScopeMode === 'aggregated'
      ...
      : selectedBookAccountId
        ? [selectedBookAccountId]
        : [];
$: visibleOrderbookHubIds = swapRouteMode !== 'cross' && orderbookScopeMode === 'aggregated' ? orderbookHubIds : [];
$: activeOrderbookRelayUrl = activeBookHubId ? orderbookRelayUrlForHub(activeBookHubId) : '';
$: orderbookDepth = orderbookScopeMode === 'aggregated' ? AGGREGATED_ORDERBOOK_DEPTH : SELECTED_ORDERBOOK_DEPTH;
...
function jurisdictionLabelForAssetKey(assetKey: string): string {
  ...
  const sourceRef = String(getReplicaJurisdictionRef(currentReplica) || '')
    .trim()
    .toLowerCase();
  const targetRef = String(selectedCrossTarget?.targetJurisdictionRef || '')
    .trim()
    .toLowerCase();
...
$: selectedRouteOption =
  visibleRouteOptions.find((option) => option.value === liveSelectedRouteValue) ||
  (selectedRouteOptionOverride?.value === liveSelectedRouteValue && routeMatchesCurrentSource(selectedRouteOptionOverride)
    ? selectedRouteOptionOverride
    : null) ||
...
$: showManualRouteRecommendation =
  ...
  (orderbookSnapshot.sourceStatus === 'no-market' || orderbookSnapshot.sourceStatus === 'empty' || orderbookSnapshot.sourceStatus === 'error');
$: sourceJurisdictionLabel = getReplicaJurisdictionName(currentReplica) || 'Current';
$: targetJurisdictionLabel = swapRouteMode === 'cross' && selectedCrossTarget ? selectedCrossTarget.targetJurisdiction : sourceJurisdictionLabel;
...
$: routePathLabel = swapRouteMode === 'cross' ? `${routePathSourceLabel} -> ${routePathTargetLabel}` : routePathSourceLabel;
...
function buildSourceEntityOptions(
  ...
  currentEntityId = String(tab.entityId || '')
    .trim()
    .toLowerCase(),
): SourceEntityOption[] {
  const options = view.localReplicas
    .map((candidate) => {
      const entityId = String(candidate.entityId || candidate.state?.entityId || '')
        .trim()
        .toLowerCase();
      const signerId = String(candidate.signerId || '')
        .trim()
        .toLowerCase();
  ...
  return options.sort((a, b) => {
    const current = String(currentEntityId || '')
      .trim()
      .toLowerCase();
...
function getProfileJurisdictionName(profile: Profile | undefined | null): string {
...
function findHubProfilesForJurisdiction(jurisdictionName: string): Profile[] {
  const normalized = String(jurisdictionName || '')
    .trim()
    .toLowerCase();
  if (!normalized) return [];
...
function buildCrossTargetOptions(
  ...
  for (const candidate of view.localReplicas) {
    const targetEntityId = String(candidate.entityId || candidate.state?.entityId || '')
      .trim()
      .toLowerCase();
...
function buildRouteOptions(
  ...
  const sourceHubEntityId = String(selectedHubEntityId || '')
    .trim()
    .toLowerCase();
...
function routeMatchesCurrentSource(route: SwapRouteOption | null | undefined): boolean {
  ...
  const currentSourceEntityId = String(sourceEntityIdValue || '')
    .trim()
    .toLowerCase();
  const routeSourceEntityId = String(route.sourceEntityId || '')
    .trim()
    .toLowerCase();
  if (currentSourceEntityId && routeSourceEntityId && currentSourceEntityId !== routeSourceEntityId) return false;
  const currentSourceRef = String(getReplicaJurisdictionRef(currentReplica) || '')
    .trim()
    .toLowerCase();
  const routeSourceRef = String(route.sourceJurisdictionRef || '')
    .trim()
    .toLowerCase();
  if (currentSourceRef && routeSourceRef && currentSourceRef !== routeSourceRef) return false;
  if (route.mode === 'cross') {
    const routeTargetEntityId = String(route.targetEntityId || '')
      .trim()
      .toLowerCase();
    const routeTargetRef = String(route.targetJurisdictionRef || '')
      .trim()
      .toLowerCase();
...
function buildCommittedRouteSelectionFromDom(
  ...
  const targetEntityId = String(targetEntityIdRaw || '')
    .trim()
    .toLowerCase();
  const targetHubEntityId = String(targetHubEntityIdRaw || '')
    .trim()
    .toLowerCase();
  if (!targetEntityId || !targetHubEntityId) return null;
  const targetReplica = findReplicaByEntityId(targetEntityId);
  const sourceHubEntityId = String(activeOrderAccountId || selectedBookAccountId || createOrderAccountId || '')
    .trim()
    .toLowerCase();
  ...
  const targetJurisdictionRef =
    String(existingTarget?.targetJurisdictionRef || getReplicaJurisdictionRef(targetReplica) || targetJurisdiction).trim() || targetJurisdiction;
  const label = String(
...
function tradingPairsForHub(hubId: string): Array<{ baseTokenId: number; quoteTokenId: number; pairId: string }> {
  const normalizedHubId = String(hubId || '')
    .trim()
    .toLowerCase();
  const hubReplica = normalizedHubId ? findReplicaByEntityId(normalizedHubId) : null;
  const configuredPairs = Array.isArray(hubReplica?.state?.swapTradingPairs) ? hubReplica.state.swapTradingPairs : [];
...
function buildPanelRoutedRouteCandidates(
  mode = swapRouteMode,
  ...
  void _quoteNonce;
  return buildRoutedRouteCandidates({
    mode,
    target,
    sourceHubId: String(sourceHubInput || '')
      .trim()
      .toLowerCase(),
...
function buildCrossOrderbookPairOptions(): SwapOrderbookPairOption[] {
  ...
  const routes = swapRouteMode === 'cross' && selectedRouteOption?.mode === 'cross' ? [selectedRouteOption] : [];
...
function buildReverseCrossRouteSelection(): {
  ...
  const sourceEntityId = String(target.targetEntityId || '')
    .trim()
    .toLowerCase();
  const sourceHubEntityId = String(target.targetHubEntityId || '')
    .trim()
    .toLowerCase();
  const targetEntityId = String(route.sourceEntityId || sourceEntityIdValue || '')
    .trim()
    .toLowerCase();
  const targetHubEntityId = String(route.sourceHubEntityId || activeOrderAccountId || '')
    .trim()
    .toLowerCase();
  ...
  const targetSignerId = String(targetReplica.signerId || '')
    .trim()
    .toLowerCase();
...
function flipSwapTokens(): void {
  ...
  const nextAmountInput = nextGiveAmount > 0n ? formatAmountForInput(nextGiveAmount, nextGiveToken) : '';
  const isCrossReverse = swapRouteMode === 'cross';
  const currentPriceTicks =
    selectedOrderLevel?.inputPriceTicks && selectedOrderLevel.inputPriceTicks > 0n
      ? selectedOrderLevel.inputPriceTicks
      : selectedOrderLevel?.priceTicks && selectedOrderLevel.priceTicks > 0n
        ? selectedOrderLevel.priceTicks
...
function handleSwapWindowClick(event: MouseEvent): void {
  const globalSuppressUntil =
    typeof window !== 'undefined' ? (window as Window & { __xlnRouteMenuSuppressWindowClickUntil?: number }).__xlnRouteMenuSuppressWindowClickUntil || 0 : 0;
  ...
  const clickedInsideMenuRoot = path.some((entry) => entry instanceof HTMLElement && Boolean(entry.closest('[data-swap-menu-root]')));
...
function selectedTokenSymbol(tokenIdValue: number, tokenIdRaw: string, options: Array<{ tokenId: number; symbol: string }>): string {
  const raw = String(tokenIdRaw || '').trim();
  const rawTokenId = Number.parseInt(raw, 10);
  const selectedOption =
    options.find((token) => String(token.tokenId) === raw) ||
...
function hasTokenInAccount(counterpartyEntityId: string, tokenIdValue: number): boolean {
  return readAccountCapacityForReplica(currentReplica, sourceEntityIdValue, resolveCounterpartyId(counterpartyEntityId), tokenIdValue)?.tokenActive ?? false;
}
...
function findReplicaByEntityId(entityId: string, view: SwapPanelRuntimeView = swapRuntimeView): EntityReadView | null {
  ...
  return (
    view.localReplicas.find(
      (candidate) =>
        String(candidate.entityId || candidate.state?.entityId || '')
          .trim()
          .toLowerCase() === normalized,
    ) || null
  );
...
$: {
  ...
  canAutoPrepareCrossInboundCapacity = Boolean(swapRouteMode === 'cross' && selectedCrossTarget && needsCrossTargetCreditSetup);
}
$: formattedAvailableGiveAmount =
  Number.isFinite(giveToken) && giveToken > 0 ? formatAmount(availableGiveCapacity, giveToken) : availableGiveCapacity.toString();
$: formattedAvailableGive = Number.isFinite(giveToken) && giveToken > 0 ? `${formattedAvailableGiveAmount} ${giveTokenSymbol}` : formattedAvailableGiveAmount;
$: {
  const nextAutoAmountContextSignature = [sourceEntityIdValue, activeOrderAccountId, giveToken].join('::');
...
$: crossSetupCreditLimitLabel =
  crossAutoInboundCreditTarget !== null && Number.isFinite(wantToken) && wantToken > 0
    ? `${formatAmount(crossAutoInboundCreditTarget, wantToken)} ${wantTokenSymbol}`
    : '';
$: crossSetupCreditIncreaseLabel =
  crossAutoInboundCreditTarget !== null && crossAutoInboundCreditTarget > crossCurrentPeerCreditLimit
...
function validateCrossSwapForm(
  ...
  const hasSourceAccount = input.accountIds.some((id) => String(id || '').toLowerCase() === String(input.counterpartyId || '').toLowerCase());
  ...
  const targetHasToken = hasTokenInReplicaAccount(targetReplica, target.targetEntityId, target.targetHubEntityId, input.wantToken);
  if (!targetHasToken && !allowAutoPrepareTargetInbound) return 'Target inbound token is not active. Enable auto-extend.';
  const targetInCapacity = readInCapacityForReplica(targetReplica, target.targetEntityId, target.targetHubEntityId, input.wantToken);
  if (input.wantAmount > targetInCapacity) {
    if (allowAutoPrepareTargetInbound) return '';
    const formattedTargetIn =
      Number.isFinite(input.wantToken) && input.wantToken > 0
...
function buildSwapValidationInput(
  ...
  const liveFormattedAvailableGive =
    Number.isFinite(candidateGiveToken) && candidateGiveToken > 0
  ...
  const liveFormattedAvailableWantIn =
    Number.isFinite(candidateWantToken) && candidateWantToken > 0
...
$: swapPreparationError =
  giveAmount > 0n && limitPriceTicks !== null && limitPriceTicks > 0n && parsedOrderbookPair && !preparedOrder
...
$: swapDisabledReason =
  ...
  (swapRouteMode === 'cross'
    ? validateCrossSwapForm(
        buildSwapValidationInput(String(activeOrderAccountId || ''), giveToken, wantToken, canonicalGiveAmount, canonicalWantAmount, canonicalPriceTicks),
        selectedCrossTarget,
...
$: swapActionDisabledReason =
  swapRouteMode === 'same' && isInboundCapacityValidationError(swapDisabledReason) && canAutoPrepareInboundCapacity ? '' : swapDisabledReason;
$: autoCapacityNote = (() => {
...
$: swapSubmitLabel = placingSwapOffer ? 'Submitting swap...' : `Swap ${giveTokenSymbol} to ${wantTokenSymbol}`;
$: if (swapRouteMode !== 'cross' && selectedOrderLevel && orderbookScopeMode !== 'aggregated' && selectedOrderLevel.accountId !== selectedBookAccountId) {
  selectedOrderLevel = null;
}
function computeOrderAmountSelection(percent: number): {
  ...
  const requantized = requantizeAtLimitPrice(levelGiveTokenId, levelWantTokenId, rawGive, explicitPriceTicks);
...
function commitOrderbookCrossRoute(option: SwapOrderbookPairOption): boolean {
  const route =
    visibleRouteOptions.find((candidate) => candidate.value === option.routeValue) ||
    (selectedRouteOption?.value === option.routeValue && routeMatchesCurrentSource(selectedRouteOption) ? selectedRouteOption : null);
  const routeIsCurrentSelected = selectedRouteOption?.value === option.routeValue && swapRouteMode === 'cross';
  const target =
    crossTargetOptions.find((candidate) => candidate.value === option.routeValue) ||
    (selectedCrossTargetOverride?.value === option.routeValue ? selectedCrossTargetOverride : null);
  ...
  if (
    String(option.sourceJurisdictionRef || '')
      .trim()
      .toLowerCase() !==
      String(route.sourceJurisdictionRef || '')
        .trim()
        .toLowerCase() ||
    String(option.targetJurisdictionRef || '')
      .trim()
      .toLowerCase() !==
      String(route.targetJurisdictionRef || '')
        .trim()
        .toLowerCase()
  ) {
...
function handleOrderbookLevelClick(event: CustomEvent<SwapOrderbookLevelClickDetail>) {
  ...
  selectedOrderLevel = {
    ...
    inputPriceTicks: parseDisplayPriceTicks(typeof event.detail.displayPrice === 'string' ? event.detail.displayPrice : '', priceTicks),
    baseTokenId: pair.baseTokenId,
    quoteTokenId: pair.quoteTokenId,
...
$: if (!hasAutoSuggestedInitialPrice && !hasUserEditedPriceInput && !priceRatioInput && orderbookSnapshot.updatedAt > 0) {
...
function prepareCanonicalOrder(rawGiveAmount: bigint, rawWantAmount: bigint): PreparedSwapOrderLike | null {
  ...
  const explicitPriceTicks =
    selectedOrderLevel?.inputPriceTicks && selectedOrderLevel.inputPriceTicks > 0n ? selectedOrderLevel.inputPriceTicks : limitPriceTicks;
  if (explicitPriceTicks && explicitPriceTicks > 0n) {
    const requantized = requantizeAtLimitPrice(giveToken, wantToken, rawGiveAmount, explicitPriceTicks);
...
$: parsedOrderbookPair = (() => {
  ...
  return selectedPair ? { baseTokenId: selectedPair.baseTokenId, quoteTokenId: selectedPair.quoteTokenId } : null;
})();
$: orderbookPairId = swapRouteMode === 'cross' && activeCrossMarket ? activeCrossMarket.venueId : selectedPair?.pairId || '1/2';
$: orderMode = parsedOrderbookPair
  ? swapRouteMode === 'cross' && activeCrossMarket
    ? activeCrossMarket.sourceIsBase
      ? 'sell-base'
      : 'buy-base'
    : tradeSide
  : 'none';
$: {
  const nextPriceContextSignature = [swapRouteMode, liveSelectedRouteValue, activeBookHubId, orderbookPairId, giveToken, wantToken, orderMode].join('::');
...
$: orderbookPairSelectValue = (() => {
  if (swapRouteMode === 'cross') {
    const exactCrossValue = selectedRouteOption
      ...
      : '';
    return (
      orderbookPairOptions.find((option) => option.value === exactCrossValue)?.value ||
  ...
  return (
    orderbookPairOptions.find((option) => option.value === exactSameValue)?.value ||
...
$: preparedOrder = (activeXlnFunctions, giveToken, wantToken, selectedOrderLevel, limitPriceTicks, prepareCanonicalOrder(giveAmount, wantAmount));
...
function getOfferSideLabelForPanel(offer: OfferLike): 'Ask' | 'Bid' {
  return offerSideLabel(offer, resolvePairOrientation);
}
function getOfferPriceTicksForPanel(offer: OfferLike): bigint {
  return offerPriceTicks(offer, computeSwapPriceTicksSafe);
}
function getRemainingOfferUsdForPanel(offer: SwapOfferLike): number {
  return remainingOfferUsd(offer, (tokenIdValue) => activeXlnFunctions?.getTokenInfo?.(tokenIdValue));
}
function isDustOpenOfferForPanel(offer: SwapOfferLike): boolean {
...
$: ownOrderbookEntityIds = Array.from(
  new Set(
    [sourceEntityIdValue, ...(selectedCrossTarget?.targetEntityId ? [selectedCrossTarget.targetEntityId] : [])]
      .map((id) =>
        String(id || '')
          .trim()
          .toLowerCase(),
...
$: filteredClosedOrderViews =
  closedOrderStatusFilter === 'all'
    ? Array.isArray(closedOrderViews)
      ? closedOrderViews
      : []
  ...
  if (closed) {
    ...
    if (closed.status === 'filled' && closed.filledPercent >= 99.99) {
      ...
      const improvementNote =
        closed.priceImprovementAmount > 0n
          ? ` with ${formatPriceImprovement(closed.priceImprovementAmount, closed.priceImprovementTokenId)} price improvement`
          : '';
      const feeNote = closed.feeAmount > 0n ? `. Fee: ${formatSwapFee(closed.feeAmount, closed.feeTokenId)}` : '';
      toasts.success(`Order placed and fully filled${improvementNote}${feeNote}`);
    } else if (closed.status === 'partial') {
      if (stpBlockingOrderId) {
        const feeNote = closed.feeAmount > 0n ? ` Fee: ${formatSwapFee(closed.feeAmount, closed.feeTokenId)}.` : '';
        toasts.info(
          `Swap partially filled (${closed.filledPercent.toFixed(2)}%). Remaining quantity was canceled to avoid matching your own order ${stpBlockingOrderId.slice(-8)}.${feeNote}`,
        );
      } else {
        const improvementNote =
          closed.priceImprovementAmount > 0n
...
async function placeSwapOffer() {
  ...
  try {
    ...
    const clickedPrepared = selectedOrderLevel
      ? requantizeAtLimitPrice(giveToken, wantToken, giveAmount, selectedOrderLevel.inputPriceTicks || selectedOrderLevel.priceTicks)
      : null;
    ...
    const explicitSubmitPriceTicks = parseDisplayPriceTicks(priceRatioInput, selectedOrderLevel?.inputPriceTicks || prepared.priceTicks);
...
>
  ...
  <SwapOrderList
    ...
    {resolvePairOrientation}
    {offerLifecycleKey}
    offerSideLabel={getOfferSideLabelForPanel}
    offerPriceTicks={getOfferPriceTicksForPanel}
    isDustOpenOffer={isDustOpenOfferForPanel}
    remainingOfferUsd={getRemainingOfferUsdForPanel}
    {formatPriceTicks}
    {formatAmount}
