...
static int run_metal(
    ...
    // This public table contains no secret material. Build from the algorithm,
    // never from an external cache, and wait/check before any derivation.
    id<MTLBuffer> references = [device newBufferWithLength:2u * 65536u * sizeof(uint)
                                                   options:MTLResourceStorageModeShared];
    id<MTLFunction> table_function = [library newFunctionWithName:@"build_reference_table"];
    id<MTLComputePipelineState> table_pipeline = table_function == nil ? nil
        : [device newComputePipelineStateWithFunction:table_function error:&error];
    if (references == nil || table_pipeline == nil) return -1;
    ...
    if (table_command == nil || table_encoder == nil) return -1;
    [table_encoder setComputePipelineState:table_pipeline];
    [table_encoder setBuffer:references offset:0 atIndex:0];
    [table_encoder dispatchThreadgroups:MTLSizeMake(1024u, 1u, 1u)
                 threadsPerThreadgroup:MTLSizeMake(32u, 1u, 1u)];
    [table_encoder endEncoding];
    [table_command commit];
    [table_command waitUntilCompleted];
    if (table_command.status != MTLCommandBufferStatusCompleted) return -1;
    ...
    for (uint32_t first = 0u; first < shard_count; first += workers) {
        ...
        [encoder setBuffer:references offset:0 atIndex:2];
