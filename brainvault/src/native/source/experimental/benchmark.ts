...
import { combineShardsWithParams, bytesToHex, factorForShardCount } from '../../../core/index.ts';
import { BRAINVAULT_V1, BRAINVAULT_V1_SPEC_ID, createShardSalt } from '../../../core/primitives/spec.ts';
...
const cpuExecutable = [
  ...
  `${import.meta.dir}/../c/brainvault-argon2-oversubscribed`,
].find(candidate => existsSync(candidate)) ?? `${import.meta.dir}/../c/brainvault-argon2-oversubscribed`;
const metalExecutable = `${import.meta.dir}/build/brainvault-argon2-metal`;
if ((mode === 'cpu' || mode === 'hybrid' || mode === 'parity') && !existsSync(cpuExecutable)) {
  throw new Error('BRAINVAULT_METAL_CPU_EXECUTABLE_MISSING:run make -C ../c oversubscribed');
}
...
async function benchmark(): Promise<void> {
  ...
  try {
    ...
    if (shardCount === 10_000 && root !== '5557e8b96514ba45d0f3af0450616c68d41625731a8de9fbe54046cce1de0298') {
      throw new Error(`BRAINVAULT_EXPERIMENT_ROOT_MISMATCH:${root}`);
    }
    console.log(
      JSON.stringify(
        {
